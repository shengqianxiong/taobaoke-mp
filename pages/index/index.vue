<template>
	<view class="index-content" style="background: white">
		<scroll-view scroll-x class="nav selectTop" scroll-with-animation :scroll-left="scrollLeft">
			<view class="cu-item" :class="index == TabCur ? 'text-green cur' : ''" v-for="(item, index) in category" :key="index" @tap="tabSelect" :data-id="index">
				<text
					:style="
						index == TabCur
							? 'background: #FFFFFF;padding: 4rpx 20rpx 4rpx 20rpx;border-radius: 32rpx;margin-right: -20rpx'
							: 'background: transparent;color: #FFFFFF;margin-right: -20rpx'
					"
				>
					{{ item.name }}
				</text>
		...
		<!-- banner板块 -->
		<view v-if="TabCur == 0">
			<view class="swiper" style="padding-top: 90upx;background: white">
				<swiper
					...
					style="height: 240rpx;padding: 18rpx 16rpx 8rpx 16rpx"
				>
					<block v-for="(item, index3) in banners" :key="index3">
						<swiper-item class="swiper-wrapper" @tap="toGoodsInfo(item.url)">
							<image :src="item.image_url" style="width: 100%;height: 220rpx;border-radius: 32rpx;" mode="scaleToFill"></image>
						</swiper-item>
					</block>
				</swiper>
			</view>
			...
			<view style="margin: 16upx;" v-if="navlist.length>0">
				<view class="index-navlist s-grids" style="padding-bottom: 20upx;background: white">
					<view class="is-col-1-5 is-center" style="margin-top: 16upx;" v-for="(nav, index5) in navlist" :key="index5" @tap="toNavList(nav.url, nav.title)">
						<view class="has-pdtb-5">
			...
			<swiper
				...
				style="height: 60upx;margin-left: 32upx;margin-right: 32upx;line-height: 60upx;"
			>
			...
			<view style="display: flex;padding: 16upx;" v-if="navlist3.length>0">
				...
				<view style="width: 50%;padding-left: 10upx;height:310upx;border-radius:12upx;box-shadow: rgba(183, 183, 183, 0.3) 0px 1px 10px;" @tap="toNavList(navlist3[1].url)">
			...
			<view v-if="navlist3.length>0" style="margin-left: 16upx;margin-right: 16upx;display: flex;height: 250upx;justify-content: space-around;box-shadow: rgba(183, 183, 183, 0.3) 0px 1px 10px;">
				...
				<view style="width: 32%;padding-left: 20upx;border-radius: 6px;box-shadow: rgba(183, 183, 183, 0.3) 0px 1px 10px;" @tap="toNavList(navlist3[4].url)">
				...
				<view style="width: 32%;padding-left: 20upx;border-radius: 6px;box-shadow: rgba(183, 183, 183, 0.3) 0px 1px 10px;" @tap="toNavList(navlist3[3].url)">
			...
			<view style="display: flex;justify-content: space-around;margin-left: 16upx;margin-right: 16upx;" v-if="juhuasuanlist.length>0&&wanghong">
				<view>
					<view v-for="(g, index7) in juhuasuanlist" :key="index7" class="box-float" >
						<image lazy-load='true' fade-show='true' style="border-radius: 16upx;width: 214upx;height: 214upx;margin-right: 20upx;" class="image" :src="g.itempic" @click="toGoodsInfos(g.itemid)" mode="scaleToFill"></image>
		...
		<view class="index-coupon has-pd-10" style="margin:90upx 8upx 90upx 8upx ;background: #FFFFFF" v-if="TabCur != 0">
				...
				<view class="index-coupon has-bg-white has-pd-10">
					<view class="goods-list" v-if="category[TabCur].orderList.length > 0">
						<orange-goods-card-home
							         v-for="(item,index) in category[TabCur].orderList"
							        :index="index%2"
		...
		<!-- 加载更多提示 -->
		<view class="s-col is-col-24"><load-more :loadingType="category[TabCur].loadingType" :contentText="contentText"></load-more></view>

		<!-- 加载更多提示 -->
		<empty v-if="category[TabCur].orderList.length === 0 && category[TabCur].loadingType == 0" des="暂无数据"></empty>

		<view class="scroll_top" @tap="topScrollTap" v-bind:class="[scrollTop != 0 ? 'active' : '', '']" style="bottom: 56px;"><text class="iconfont icon-shangla"></text></view>
...
<script>
	...
	export default {
		onShareAppMessage(res) {
			return {
				title: '快戳我领免单！你购物我买单',
				path: '/pages/index/index?invitation=' + this.invitation
			};
		},
		components: {
			simpleModal,
...
// 新加
	.nablist-1{
		color:#333333;font-size:32upx;font-weight: bold;padding-top: 16upx;
	}
	.box-float {
		width: 33%;
		float: left;
...
.selectTop {
	...
	position: fixed;
	top: 0;
	background: -webkit-linear-gradient(left, #e10a07 0, #e10a07 100%);
	background: -o-linear-gradient(left, #e10a07 0, #e10a07 100%);
	background: -ms-linear-gradient(left, #e10a07 0, #e10a07 100%);
	background: -webkit-gradient(linear, right top, left top, color-stop(0, #e10a07), to(#e10a07));
	background: -o-linear-gradient(right, #e10a07 0, #e10a07 100%);
